...
app.layout = html.Div([
    html.Div([
        html.Div([
            html.Img(src=app.get_asset_url('coronaboard-1.jpg'),
                     id = 'corona-image',
                     style={'height': '120px',
                            'width': 'auto',
                            'margin-bottom': '15px'})
        ...
        html.Div([
            html.Div([
                ...
                html.H5('전세계 코로나19 실시간 상황판', style={'margin-bottom': '0px', 'color': 'white'})
            ])
...
html.Div([
            html.H6(children='완치자 수',
                    style={'textAlign': 'center',
                           'color': 'white'}),
            html.P(f"{covid_data_1['recovered'].iloc[560]:,.0f}",
                    style={'textAlign': 'center',
                           'color': 'green',
                           'fontSize': 40}),
            html.P('new: ' + f"{covid_data_1['recovered'].iloc[560] - covid_data_1['recovered'].iloc[559]:,.0f}"
                   + ' (' + str(round(((covid_data_1['recovered'].iloc[560] - covid_data_1['recovered'].iloc[559]) /
                                   covid_data_1['recovered'].iloc[560]) * 100, 2)) + '%)',
                   style={'textAlign': 'center',
                          'color': 'green',
                          'fontSize': 15,
                          'margin-top': '-18px'})
...
html.Div([
            html.H6(children='활동 감염자 수',
                    style={'textAlign': 'center',
                           'color': 'white'}),
            html.P(f"{covid_data_1['active'].iloc[-1] - covid_data_1['recovered'].iloc[560]:,.0f}",
                    style={'textAlign': 'center',
                           'color': '#e55467',
                           'fontSize': 40}),
            html.P('new: ' + f"{covid_data_1['active'].iloc[-1] - covid_data_1['active'].iloc[-2] - covid_data_1['recovered'].iloc[560]:,.0f}"
                   + ' (' + str(round(((covid_data_1['active'].iloc[-1] - covid_data_1['active'].iloc[-2] - covid_data_1['recovered'].iloc[560]) /
    ...
    html.Div([
        html.Div([
            ...
            html.P('현재 날짜: ' + ' ' + str(covid_data['date'].iloc[-1].strftime('%B %d, %Y')),
...
def update_graph(w_countries):
    ...
    return {
        ...
        'layout': go.Layout(
            title={'text': '최근 30일 일일 확진자 수: ' + (w_countries),
                   'y': 0.93,
                   'x': 0.5,
                   'xanchor': 'center',
                   'yanchor': 'top'},
